    ...
    def to_prompt_friendly(self, top_k: int = 5) -> str:
        ...
        for edge in sorted(self.edges, key=lambda x: -x.weight)[:top_k]:
            lines.append(
                ...
                f"(weight: {edge.weight:.2f}, overlap: {edge.evidence.value_overlap or 0:.2f})"
            )
        return "\n".join(lines)