...
class ColumnSemanticVector(BaseModel):
    ...
    def similarity_to(self, other: "ColumnSemanticVector") -> float:
        ...
        # ===== 8. 新增：列名语义相似度（基于 name_embedding） =====
        if self.name_embedding and other.name_embedding:
            from numpy.linalg import norm
            import numpy as np
            v1 = np.array(self.name_embedding)
            v2 = np.array(other.name_embedding)
            norm1 = norm(v1)
            norm2 = norm(v2)
            if norm1 > 0 and norm2 > 0:
                name_sim = float(np.dot(v1, v2) / (norm1 * norm2))
                scores.append(max(0.0, name_sim))
            else:
                scores.append(0.0)
            weights.append(0.12)
        else:
            # 没有 embedding 时，fallback 到模式签名
            scores.append(pattern_sim)
            weights.append(0.12)
        
        # 9. 平均长度相似度（新增）
        if self.avg_length is not None and other.avg_length is not None:
            length_sim = 1.0 - min(abs(self.avg_length - other.avg_length) / max(self.avg_length, other.avg_length, 1), 1.0)
